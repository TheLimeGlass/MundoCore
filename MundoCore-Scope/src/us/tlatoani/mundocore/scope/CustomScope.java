package us.tlatoani.mundocore.scope;
...
public abstract class CustomScope extends Condition {
	...
	public static DocumentationBuilder.Scope registerScope(Class<? extends CustomScope> conditionClass, String... patterns) {
		...
		return new DocumentationBuilder.Scope(Registration.getCurrentCategory(), patterns).requiredPlugins(Registration.getCurrentRequiredPlugins());
	}
	...
	private static void registerImmediateScopes(Trigger trigger) {
	...
	protected static void getScopes() {
		if (!getScopesWasRun) {
			try {
	...
	private static Conditional getScope(TriggerSection parent, CustomScope scope) {
